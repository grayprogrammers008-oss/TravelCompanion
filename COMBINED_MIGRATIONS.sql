...
-- ============================================
-- PRE-MIGRATION FIX: Add missing columns
-- ============================================
-- This fixes schema mismatches when tables existed before BASE_SCHEMA_MINIMAL.sql
-- Using simple ALTER TABLE ... ADD COLUMN IF NOT EXISTS (PostgreSQL 9.6+)

-- TRIPS TABLE: Add missing columns
ALTER TABLE public.trips ADD COLUMN IF NOT EXISTS destination TEXT DEFAULT 'Unknown';
ALTER TABLE public.trips ADD COLUMN IF NOT EXISTS is_public BOOLEAN DEFAULT false;
ALTER TABLE public.trips ADD COLUMN IF NOT EXISTS cost DOUBLE PRECISION;
ALTER TABLE public.trips ADD COLUMN IF NOT EXISTS is_completed BOOLEAN DEFAULT false;
ALTER TABLE public.trips ADD COLUMN IF NOT EXISTS completed_at TIMESTAMPTZ;
ALTER TABLE public.trips ADD COLUMN IF NOT EXISTS rating DOUBLE PRECISION;
ALTER TABLE public.trips ADD COLUMN IF NOT EXISTS currency TEXT DEFAULT 'INR';
ALTER TABLE public.trips ADD COLUMN IF NOT EXISTS cover_image_url TEXT;

-- CHECKLIST_ITEMS TABLE: Add missing columns
ALTER TABLE public.checklist_items ADD COLUMN IF NOT EXISTS name TEXT;
ALTER TABLE public.checklist_items ADD COLUMN IF NOT EXISTS content TEXT DEFAULT 'Item';
...
CREATE OR REPLACE FUNCTION public.admin_update_trip(
  ...
  p_cost DOUBLE PRECISION DEFAULT NULL,
...
CREATE OR REPLACE FUNCTION public.get_public_trips(
    p_search TEXT DEFAULT NULL,
    p_destination TEXT DEFAULT NULL,
    p_min_cost DOUBLE PRECISION DEFAULT NULL,
    p_max_cost DOUBLE PRECISION DEFAULT NULL,
...
BEGIN
    ...
    WHERE t.is_public = true
        ...
        AND (p_min_cost IS NULL OR t.cost >= p_min_cost)
        AND (p_max_cost IS NULL OR t.cost <= p_max_cost)
...
BEGIN
  ...
  -- 1. Delete messages associated with trip conversations first
...
-- SKIP: -- Rename the column
-- SKIP: ALTER TABLE public.trips
-- SKIP: RENAME COLUMN budget TO cost;
