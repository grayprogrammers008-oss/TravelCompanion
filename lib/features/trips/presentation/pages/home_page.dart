...
class _HomePageState extends ConsumerState<HomePage>
    ...
    return Scaffold(
      ...
      body: MeshGradientBackground(
        ...
        child: RefreshIndicator(
          ...
          SliverToBoxAdapter(
            child: _buildExpenseSummaryCard(context, ref, themeData),
          ),
  ...
  /// Build expense summary card for home dashboard
  Widget _buildExpenseSummaryCard(BuildContext context, WidgetRef ref, AppThemeData themeData) {
    final expenseSummaryAsync = ref.watch(expenseSummaryProvider);

    return expenseSummaryAsync.when(
      data: (summary) {
        // Don't show card if no expenses at all
        if (summary.totalAll == 0) return const SizedBox.shrink();
                    ...
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: AppTheme.spacingMd),
                          // This month's spending (primary metric)
                          Text(
                            _formatCurrencyCompact(summary.thisMonthSpending),
                            style: const TextStyle(
                              color: Colors.white,
                              fontSize: 32,
                              fontWeight: FontWeight.w700,
                          ...
                          if (summary.totalAll > summary.thisMonthSpending) ...[
                            const SizedBox(height: 4),
                            Text(
                              'All time: ${_formatCurrencyCompact(summary.totalAll)}',
                              style: TextStyle(
