-- ============================================================================
-- DATA MIGRATION EXPORT SCRIPT (FIXED - Handles NULL Dates)
-- ============================================================================
--
...
--
-- FIXES:
-- - Handles NULL dates by providing default values
-- - NULL start_date → current date
-- - NULL end_date → start_date + 1 day
-- ============================================================================

-- Header
SELECT '-- ============================================================================' as import_script;
SELECT '-- DATA IMPORT SCRIPT' as import_script;
SELECT '-- Generated from OLD database (FIXED DATES)' as import_script;
...
-- ============================================================================
-- SECTION 3: TRIPS DATA (FIXED - Handles NULL dates)
-- ============================================================================
...
SELECT format(
    'INSERT INTO public.trips (id, name, destination, start_date, end_date, created_by) VALUES (%L, %L, %L, %L, %L, %L) ON CONFLICT (id) DO NOTHING;',
    id,
    name,
    COALESCE(destination, ''),
    -- Fix NULL start_date: use current date if NULL
    COALESCE(start_date, CURRENT_DATE),
    -- Fix NULL end_date: use start_date + 1 day if NULL
    COALESCE(end_date, COALESCE(start_date, CURRENT_DATE) + INTERVAL '1 day'),
    created_by
) as import_script
FROM public.trips
ORDER BY id;
...
SELECT '-- NOTE: NULL dates were replaced with default values' as import_script;
SELECT '-- ============================================================================' as import_script;
